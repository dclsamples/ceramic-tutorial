			...
			<div className={styles.mt2}>
				{record.content ? (
					<div className={styles.flexCol}>
						...
						<p>The above name was loaded from Ceramic Network. Try updating it below.</p>
					</div>
				) : (
					<p className={styles.pCenter}>
						You do not have a profile record attached to your 3ID.
						<br /> Create a basic profile by setting a name below.
					</p>
				)}
			</div>
					...
					{connection.status === "connected" ? (
						<div>
							<p className={styles.subtitle}>
								Your 3ID is <br />
								<span className={styles.threeId}>{connection.selfID.id}</span>
							</p>
							<RecordSetter />
						</div>
					) : (
